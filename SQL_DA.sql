...
from league 
group by country_id, name having count(*) > 1;

-- Inserting a duplicate for practice:-
...
select date from matches limit 10;

-- update matches set date = str_to_date(date, "%d-%m-%Y");
-- creating view for home team goal counts:-
...
group by matches.home_team_api_id having sum(matches.home_team_goal) > 3;

-- creating view for away team goal counts:-
...
group by matches.away_team_api_id having sum(matches.away_team_goal) > 3;
...
create table Winning_team_details as
select winning_team_api_id, match_api_id, team.team_long_name, match_date from
...
from matches) as A
join team on A.winning_team_api_id = team.team_api_id;

select * from Winning_team_details;
...
-- Under every country and league calculate the metrics like,
